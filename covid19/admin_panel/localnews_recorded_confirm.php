...
function confirmRecorded() {
    ...
    if( uploadNewsPhoto() ) {
        $stmt = $GLOBALS['conn']->prepare("INSERT INTO local_news (img, news_title, news_body, recorded_time, am_pm, created_time)
        VALUES (:img, :news_title, :news_body, :recorded_time, :am_pm, :created_time)");
        
        $stmt->bindParam(':img',$img);
        $stmt->bindParam(':news_title',$_POST['localnews_title']);
        $stmt->bindParam(':news_body',$_POST['localnews_body']);
        ...
        echo "<script>
            alert('Failed to record local news.');
            window.location.href = 'dashboard.php';
        </script>";
    }
}
