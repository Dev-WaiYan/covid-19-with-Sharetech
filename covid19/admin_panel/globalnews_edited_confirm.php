...
function confirmUpdate() {
    global $conn;
    if($_FILES['globalnews_main_photo']['name'] != '') {
        if(uploadNewsPhoto()) {
            global $uploaded_time;
            $stmt = $conn->prepare("UPDATE global_news SET img=:img,news_title=:news_title, news_body=:news_body WHERE id=:news_id");
        ...
        $stmt = $conn->prepare("UPDATE global_news SET news_title=:news_title, news_body=:news_body WHERE id=:news_id");
        $stmt->bindParam(':news_title',$_POST['news_title']);
        $stmt->bindParam(':news_body',$_POST['news_body']);
        $stmt->bindParam(':news_id',$_POST['news_id']);
    }
    ...
    if($stmt->execute()) {
        setLogGlobalnewsUpdated($_POST['news_id']);
        echo "
            <script>
                alert('Updated News.');
...
function confirmRemove() {
    global $conn;
    $stmt = $conn->prepare("DELETE FROM global_news WHERE id = :id");
    $stmt->bindParam(':id',$_POST['news_id']);
    if($stmt->execute()) {
        setLogGlobalnewsDeleted($_POST['news_title']);
        echo "
            <script>
